...
async def tmuter(event):  # sourcery no-metrics
    ...
    if not user:
        return
    if not reason:
        return await catevent.edit("**𓆩𓆩𖠄 ⦙ رجاء طريقه كتابه الامر خاطئه قم بروئيه قناه شروحات الاوامر : @botate56 𓆪𓆪**")
...
async def iq(event):
    ...
    if type_of_group == "قناه":
        descript = "𓆩𓆩𖠄 ⦙ هذه قناة إختبار أُنشئت بإستعمال فينوم 𓆪𓆪"
    else:
        descript = "𓆩𓆩𖠄 ⦙ هذه المجموعه إختبار أُنشئت بإستعمال فينوم 𓆪𓆪"
