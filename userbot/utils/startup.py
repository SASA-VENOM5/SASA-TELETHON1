...
async def startupmessage():
    try:
        if BOTLOG:
            Config.CATUBLOGO = await iqthon.tgbot.send_file(BOTLOG_CHATID, "https://telegra.ph/file/9fabee1e4ce0ee6f98484.jpg", caption="♛ ⦙ تـمّ اعـادة تشـغيل\n سورس فينوم ✓  :  [ 7.5 ] .\n\n♛ ⦙ للحصول على اوامر السورس\n أرسـل : (  `.اوامري`  ) \n\n♛ ⦙ لمـعرفة كيفية تغير بعض كلايش\n او صور السـورس  أرسـل  :\n (  `.مساعده`  )\n\n♛ ⦙ القناة الرسمية سورس فينوم : @votate56",                buttons=[(Button.url("مطور السورس", "https://t.me/M_2_F"),)],            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        _, groupid = await create_supergroup(            "مجموعه بوت فينوم الخاص بك", iqthon, Config.TG_BOT_USERNAME, descript        )
