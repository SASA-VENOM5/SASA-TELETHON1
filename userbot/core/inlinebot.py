...
async def inline_handler(event):  
    ...
    if query_user_id == Config.OWNER_ID or query_user_id in Config.SUDO_USERS:
        hmm = re.compile("هـمـسه (.*) (.*)")
        match = re.findall(hmm, query)
        if query.startswith("**iqthonbot"):
            buttons = [
                (
                    Button.inline("السـورس", data="stats"),
                    Button.url("الريـبو", "https://github.com/SASA-ANTAR/SASA-TELETHON"),
                )
            ]
            ...
            buttons = [Button.inline("𓆩𓆩 اظـهر الرسـالة 𓆪𓆪", data=f"secret_{timestamp}")]
            result = builder.article(
                title="𓆩𓆩 رسـالة سـرية 𓆪𓆪",
                text=f"𓆩𓆩 رسـالة تهــمس ل {sandy}, هـو فقـط مـن يـمكنه فتـحه.𓆪𓆪",
                buttons=buttons,
            )
            await event.answer([result] if result else None)
            if jsondata:
            ...
            buttons = [
                Button.inline(text="اضهــار𓆩𓆩 الــخيـارات.𓆪𓆪", data="show_pmpermit_options"),
            ]
        ...
        result = types.InputBotInlineResult(
            ...
            type="photo",
            title="iqthon",
            description="نصـب لـنـفـسـك",
            url="https://dashboard.heroku.com/new?template=https://github.com/SASA-VENOM/SASA-TELETHON",
            thumb=photo,
            content=photo,
            send_message=types.InputBotInlineMessageMediaAuto(
                reply_markup=markup, message=text, entities=msg_entities
